...
| Compute-root address reuse-distance discriminator | IMPLEMENTED_UNPROVEN | `docs/testing/FG2_ROOT_ADDRESS_REUSE_DISTANCE_HOST_2026-08-21.md`; same-source A/B versus A/B/C roots with common X/Y/Z/W QMD schedule passes host/build gates | freeze an immutable revision, then run gated control first on real Tegra; no device contacted and no hardware result yet |
...
| Render → sampled image → compute → storage image chain | BLOCKED | prior negative results plus the root-address and QMD shader-constant-cache records; fresh-root alternation advanced consumption once, while isolated QMD bit 255 did not change the resulting seed-42-retention signature | specify root reuse distance as a separate discriminator; do not infer generic root, QMD, cache, allocator, or GPU-consumption behavior |
