...
| Reused compute-QMD payload identity | PROVEN_HW | `docs/testing/FG2_QMD_UPLOAD_IDENTITY_RUN_2026-08-21.md`; complete 256-byte source/mapped payloads were identical across the first two correlated launches at reused VA `0xc7f40800` | QMD-only flush is non-discriminating and was forbidden; specify QMD-address-reuse separately |
| Two-slot compute-QMD address intervention | PROVEN_HW | `docs/testing/FG2_QMD_ADDRESS_REUSE_RUN_2026-08-21.md`; 63/63 consecutive address transitions and 64/64 exact copies/dispatch correlations retained the exact stale signature | this specific adjacent two-slot intervention was insufficient; do not generalize to other QMD, cache, allocator, or GPU-consumption behavior |
| Two-slot compute-root address intervention | PROVEN_HW | `docs/testing/FG2_ROOT_ADDRESS_REUSE_RUN_2026-08-21.md`; valid control plus 63/63 root/QMD transitions, complete copy/decode/mask/dispatch correlation, and changed seed-42-retention signature | `behavior_changed_unresolved`; isolate why fresh-root consumption advanced once but not thereafter in a separate smaller change; not a production fix |
| QMD v0.6 shader-constant-cache invalidation discriminator | PROVEN_HW | `docs/testing/FG2_QMD_SHADER_CONSTANT_CACHE_INVALIDATE_RUN_2026-08-21.md`; valid control plus causally isolated bit-255 variant retained identical seed-42 stale output across all 64 corresponding records | `specific_qmd_shader_constant_cache_invalidate_insufficient`; do not generalize beyond this controlled launch pattern; specify root reuse distance separately |
| Compute-root address reuse-distance discriminator | PROVEN_HW | `docs/testing/FG2_ROOT_ADDRESS_REUSE_DISTANCE_RUN_2026-08-21.md`; valid same-source A/B versus A/B/C pair moved first root reuse from record 3 to 4 but retained identical seed-42 stale output from record 3 through 64 | `specific_root_address_reuse_distance_change_insufficient`; reject only this tested distance increase; at most one later OpenSpec change may isolate root backing identity |
...
| Render → sampled image → compute → storage image chain | BLOCKED | prior negative results plus the root-address, QMD shader-constant-cache, and root-reuse-distance records; the A/B/C distance increase did not move the seed-42-retention onset | if pursued, specify at most one root-backing-identity discriminator separately; do not infer generic root, QMD, cache, allocator, or GPU-consumption behavior |
