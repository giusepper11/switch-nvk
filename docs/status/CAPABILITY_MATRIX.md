...
| Render → sampled image → compute → storage image chain | BLOCKED | initial/shared-layout failures plus root, flush, QMD-identity, and `docs/testing/FG2_QMD_ADDRESS_REUSE_RUN_2026-08-21.md` evidence; current root/QMD bytes and direct `PCAS` correlate exactly, and the exact stale signature survives 63/63 alternating QMD-address transitions | test compute-root GPU-address reuse as the current smallest discriminator while holding fresh-QMD behavior constant; do not infer generic root, QMD, cache, allocator, or GPU-consumption behavior |
