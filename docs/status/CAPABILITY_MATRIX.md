...
| QMD v0.6 shader-constant-cache invalidation discriminator | IMPLEMENTED_UNPROVEN | `docs/testing/FG2_QMD_SHADER_CONSTANT_CACHE_INVALIDATE_HOST_2026-08-21.md`; typed bit-255 control/variant, 64-record generated-payload test, strict selectors, correlation validator, and paired NROs pass host/build gates | run the immutable control on real Tegra first; do not interpret the variant unless the retained control signature and all causal evidence reproduce |
...
| Render → sampled image → compute → storage image chain | BLOCKED | prior negative results plus `docs/testing/FG2_ROOT_ADDRESS_REUSE_RUN_2026-08-21.md`; fresh-root alternation changed the failure from seed-5 retention to seed-42 retention after iteration 2, with complete causal correlation | specify a smaller discriminator for why alternating roots advanced consumption once but not thereafter; do not infer generic root, QMD, cache, allocator, or GPU-consumption behavior |
