...
QMD_SHADER_CONSTANT_CACHE_CONTROL="${QMD_SHADER_CONSTANT_CACHE_CONTROL:-0}"
QMD_SHADER_CONSTANT_CACHE_INVALIDATE="${QMD_SHADER_CONSTANT_CACHE_INVALIDATE:-0}"
ROOT_ADDRESS_REUSE_DISTANCE_CONTROL="${ROOT_ADDRESS_REUSE_DISTANCE_CONTROL:-0}"
ROOT_ADDRESS_REUSE_DISTANCE_VARIANT="${ROOT_ADDRESS_REUSE_DISTANCE_VARIANT:-0}"
...
case "$QMD_SHADER_CONSTANT_CACHE_CONTROL" in 0|1) ;; *) echo "QMD_SHADER_CONSTANT_CACHE_CONTROL must be 0 or 1" >&2; exit 2 ;; esac
case "$QMD_SHADER_CONSTANT_CACHE_INVALIDATE" in 0|1) ;; *) echo "QMD_SHADER_CONSTANT_CACHE_INVALIDATE must be 0 or 1" >&2; exit 2 ;; esac
if [ "$QMD_SHADER_CONSTANT_CACHE_CONTROL" = 1 ] && [ "$QMD_SHADER_CONSTANT_CACHE_INVALIDATE" = 1 ]; then
  echo "QMD shader constant-cache selectors are mutually exclusive" >&2; exit 2
fi
if { [ "$QMD_SHADER_CONSTANT_CACHE_CONTROL" = 1 ] || [ "$QMD_SHADER_CONSTANT_CACHE_INVALIDATE" = 1 ]; } &&
   { [ "$ROOT_UPLOAD_CACHE_FLUSH" = 1 ] || [ "$QMD_UPLOAD_IDENTITY" = 1 ] ||
     [ "$QMD_UPLOAD_CACHE_FLUSH" = 1 ] || [ "$QMD_ADDRESS_CONTROL" = 1 ] ||
     [ "$QMD_ADDRESS_FRESH" = 1 ] || [ "$ROOT_ADDRESS_CONTROL" = 1 ] ||
     [ "$ROOT_ADDRESS_FRESH" = 1 ]; }; then
  echo "QMD shader constant-cache experiment excludes every historical intervention selector" >&2; exit 2
fi
case "$ROOT_ADDRESS_REUSE_DISTANCE_CONTROL" in 0|1) ;; *) echo "ROOT_ADDRESS_REUSE_DISTANCE_CONTROL must be 0 or 1" >&2; exit 2 ;; esac
case "$ROOT_ADDRESS_REUSE_DISTANCE_VARIANT" in 0|1) ;; *) echo "ROOT_ADDRESS_REUSE_DISTANCE_VARIANT must be 0 or 1" >&2; exit 2 ;; esac
if [ "$ROOT_ADDRESS_REUSE_DISTANCE_CONTROL" = 1 ] && [ "$ROOT_ADDRESS_REUSE_DISTANCE_VARIANT" = 1 ]; then
  echo "root address reuse-distance selectors are mutually exclusive" >&2; exit 2
fi
if { [ "$ROOT_ADDRESS_REUSE_DISTANCE_CONTROL" = 1 ] || [ "$ROOT_ADDRESS_REUSE_DISTANCE_VARIANT" = 1 ]; } &&
   { [ "$ROOT_UPLOAD_CACHE_FLUSH" = 1 ] || [ "$QMD_UPLOAD_IDENTITY" = 1 ] ||
     [ "$QMD_UPLOAD_CACHE_FLUSH" = 1 ] || [ "$QMD_ADDRESS_CONTROL" = 1 ] ||
     [ "$QMD_ADDRESS_FRESH" = 1 ] || [ "$ROOT_ADDRESS_CONTROL" = 1 ] ||
     [ "$ROOT_ADDRESS_FRESH" = 1 ] || [ "$QMD_SHADER_CONSTANT_CACHE_CONTROL" = 1 ] ||
     [ "$QMD_SHADER_CONSTANT_CACHE_INVALIDATE" = 1 ]; }; then
  echo "root address reuse-distance experiment excludes every historical intervention selector" >&2; exit 2
fi
